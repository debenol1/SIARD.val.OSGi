package ch.kostceco.xml.impl.internal.service;
...
import ch.kostceco.xml.api.service.XmlService;
import ch.kostceco.xml.impl.internal.Activator;
...
public class XmlServiceComponent implements XmlService
{
	...
	public IStatus validate(InputStream in, File schemaLocation)
	{
		...
		{
			status = new Status(IStatus.ERROR, Activator.getContext().getBundle().getSymbolicName(), "Validierungsfehler", e);
		}
		catch (IOException e)
		{
			status = new Status(IStatus.ERROR, Activator.getContext().getBundle().getSymbolicName(), "Validierungsfehler", e);
